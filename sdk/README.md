...
| 監査ログ                                         |   ◯    |    ◯      |   ✕     |   ◯   |
| **LLM ドライバ差し替え (`llm.execute`, ADR-016)** |  ◯    |   ◯      | N/A (元から `Completer` 差替可) | ◯ |
...
**OpenAI 非互換 API (Anthropic / Bedrock / Vertex AI 等) を使いたい、あるいは
モック・キャッシュ・PII マスキングをラッパー側でかけたい場合**は、SDK の
LLM ハンドラ機能 (ADR-016) で `llm.execute` 経由にコア側 LLM 呼び出しを
委譲できる。`SLLM_ENDPOINT` は不要 (使われない):

```python
# Python
from ai_agent import Agent, AgentConfig
async with Agent(AgentConfig(binary="./agent", llm_handler=my_handler)) as agent:
    ...
```

```ts
// JS (Node)
agent.setLLMHandler(myHandler);
await agent.configure({ llm: { mode: "remote" } });
```

```swift
// Swift
let agent = Agent(config: AgentConfig(binary: bin, llmHandler: myHandler))
```

詳細は各 SDK README の「LLM ハンドラ」節と
[`docs/openrpc.json`](../docs/openrpc.json) (`llm.execute`) を参照。

ブラウザ SDK は SLLM サーバ不要。WebLLM が IndexedDB にモデルを落として
WebGPU で実行する。なお Browser SDK は元から `Completer` interface で
任意の LLM 実装を差し替え可能なので、`llm.execute` ラッパー委譲機構は不要。
...
| core → wrapper  | `llm.execute` — `llm.mode="remote"` 指定時に ChatCompletion をラッパーへ委譲 (ADR-016)                            |
...
- ADR-001 (JSON-RPC over stdio) / ADR-013 (RemoteTool アダプタ) /
  ADR-016 (`llm.execute` 逆 RPC) ほかはリポジトリルートで `/decision list` を実行
