...
**Swift SDK は別扱い**: `sdk/swift/Package.swift` にはバージョン値を書かない。
SwiftPM の慣習に従い、Git tag (例: `v0.2.0`) でバージョンを管理する。
タグを push すれば `.package(url: ..., from: "0.2.0")` で利用可能になる。
...
2. **バージョン bump** — 以下を同じ値に更新:
   ...
   - `CLAUDE.md` の `## バージョン`
   - Swift SDK (`sdk/swift`) は Package.swift を変更しない (Git tag のみ)
