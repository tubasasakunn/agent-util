...
| TypeScript SDK (Node) | `sdk/js/package.json`                                                  |
| Browser SDK           | `sdk/js-browser/package.json`                                          |
| Swift SDK 定数        | `sdk/swift/Sources/AIAgent/RawAgent.swift` の `aiAgentSDKLibraryVersion` |
...
**Swift SDK の配布**: `sdk/swift/Package.swift` にはバージョン値を書かない。
SwiftPM の慣習に従い、Git tag (例: `v0.3.0`) でバージョンを管理する。
タグを push すれば `.package(url: ..., from: "0.3.0")` で利用可能になる。
ただし、SDK 内部の `aiAgentSDKLibraryVersion` 定数は **バイナリとのハンドシェイク
比較** (ADR-018, `server.info`) に使われるため、表のとおりソースコード上の値も
他言語と同期する。
...
2. **バージョン bump** — 以下を同じ値に更新:
   ...
   - `sdk/js-browser/package.json` の `version`
   - `sdk/swift/Sources/AIAgent/RawAgent.swift` の `aiAgentSDKLibraryVersion` 定数
   - `README.md` の Latest バッジ
   - `CLAUDE.md` の `## バージョン`
   - Swift SDK (`sdk/swift`) の `Package.swift` は変更しない (Git tag で配布)
