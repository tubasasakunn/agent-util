...
- **差し替え可能な境界はインターフェースで抽象化する**: 実装が差し替わりうるコンポーネント（LLMバックエンド、MCP I/O、スキル登録、ツール実行など）は、提供側パッケージにインターフェースを定義して具体実装と分離する。複数の消費者が同じ契約に依存する場合は、消費者側ではなく提供側に共通インターフェースを置く（例: `llm.Completer`）
