...
`sdk/js-browser/package.json` / `sdk/swift/Sources/AIAgent/RawAgent.swift` の
`aiAgentSDKLibraryVersion` 定数 / `README.md` バッジ / 本セクションは常に同一値で
同期する。Swift SDK の Git tag (`v0.3.0` 等) も同じ値で打つ。
...
- ADR-017: max_turns 到達時を Result の正常リターンに変更する
- ADR-018: `server.info` ハンドシェイク RPC で SDK ⇔ バイナリ互換性を可視化する
- ADR-019: `AgentConfig.custom*` で「ハンドラ定義」と「configure 経由の有効化」を統合する
- ADR-020: `ToolScopeConfig.tool_budget` による同一ツール連続呼び出しの hard cap
- ADR-021: `JudgeConfig.builtin` で内蔵 GoalJudge を spec 文字列から組み立てる
- ADR-022: `context.status` 通知に履歴イベント情報 (`last_event` / `compaction_delta`) を載せる
- ADR-023: `llm.execute` に `session_id` と `call_index` を載せてラッパー側 KV cache を可能にする
